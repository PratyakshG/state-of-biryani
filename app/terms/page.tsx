import type { Metadata } from "next";
import Link from "next/link";

export const metadata: Metadata = {
  title: "Terms of Service | State of Biryani",
  description:
    "Read the State of Biryani terms for orders, catering inquiries, and customer communication.",
  alternates: {
    canonical: "/terms",
  },
};

export default function TermsPage() {
  return (
    <main className="min-h-screen bg-royal-black px-6 py-24 text-royal-cream lg:px-12">
      <section className="mx-auto max-w-3xl">
        <Link
          href="/"
          className="font-body text-xs uppercase tracking-[0.25em] text-royal-gold"
        >
          State of Biryani
        </Link>
        <h1 className="mt-8 font-display text-5xl font-light leading-tight sm:text-6xl">
          Terms of Service
        </h1>
        <p className="mt-6 font-body text-base leading-relaxed text-royal-cream-muted">
          State of Biryani order, catering, and corporate meal requests are
          handled based on availability, service city, preparation time, and
          confirmed communication with the customer.
        </p>
      </section>
    </main>
  );
}
